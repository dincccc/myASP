...
sqlstr="SELECT * FROM booking_record WHERE th_id=3140705237 and isdel='0' ORDER BY bk_date DESC"
rs.CursorLocation=3
...
do while not rs.eof and i<maxperpage
	...
	set conn=nothing
%>
<tr class="pageRecord"><td colspan="11"><span class="pageRecordTab"> 当前第<%=currentpage%>页 / 共<%=n%>页 | 每页<%=maxperpage%>条记录 | 共<%=totalPut%>条记录 | 转到第<input type="number" min=1 max=<%=n%> name="page" value=<%=page%> >页 <button type="button" id="submitPage">点击跳转</button> <a href=<%="?page="&currentpage-1%>>上一页</a> | <a href=<%="?page="&currentpage+1%> >下一页</a>
